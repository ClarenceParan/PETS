...
public class main {
    ...
    private static void createTablesIfNotExists(Connection conn) throws SQLException {
        // Create pets table if it doesn't exist with breed, gender, and available column
        String createPetsTable = "CREATE TABLE IF NOT EXISTS pets ("
                ...
                + "breed TEXT, "
                + "age INTEGER, "
                + "gender TEXT, "  // New column for gender
    ...
    private static void viewAvailablePets(Connection conn) throws SQLException {
        String query = "SELECT id, name, species, breed, age, gender FROM pets WHERE available = 1";
                ...
                try (PreparedStatement stmt = conn.prepareStatement(checkPetQuery)) {
                    ...
                    try (ResultSet rs = stmt.executeQuery()) {
                        if (rs.next() && rs.getBoolean("available")) {
                            ...
                            System.out.println("Pet ID is invalid or the pet is not available.");
                        }
                    }
                }
            } else {
